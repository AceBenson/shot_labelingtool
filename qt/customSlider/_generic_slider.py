...
since a it's possible to acheive a very reasonable "int slider" by
scaling input int values to some internal integer range for the QSlider,
and converting back to int when getting `value()`.  However, one still
...
class _GenericSlider(QSlider, Generic[_T]):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._maximum = 99
        self._pageStep = 1
        self._value: _T = 0  # type: ignore
        self._position: _T = 0
        self._singleStep = 1
        self._offsetAccumulated = 0
    ...
    def singleStep(self) -> int:  # type: ignore
    ...
    def pageStep(self) -> int:  # type: ignore
    ...
    def minimum(self) -> int:  # type: ignore
        return self._minimum

    def setMinimum(self, min: int) -> None:
        self.setRange(min, max(self._maximum, min))

    def maximum(self) -> int:  # type: ignore
        return self._maximum

    def setMaximum(self, max: int) -> None:
        self.setRange(min(self._minimum, max), max)

    def setRange(self, min: int, max_: int) -> None:
        oldMin, self._minimum = self._minimum, int(min)
        oldMax, self._maximum = self._maximum, int(max(min, max_))
    ...
    def setTickInterval(self, ts: int) -> None:
        self._tickInterval = max(0, ts)
        self.update()
    ...
    def _setSteps(self, single: int, page: int):
    ...
    def _pixelPosToRangeValue(self, pos: int) -> int:
        opt = self._styleOption
    ...
    def _overflowSafeAdd(self, add: int) -> int:
...
def _sliderValueFromPosition(
    min: int, max: int, position: int, span: int, upsideDown: bool = False
) -> int:
    ...
    if span > range:
        tmp = (2 * position * range + span) / (2 * span)
    else:
        div = int(range / span)
        mod = int(range % span)
        tmp = int(position * div + (2 * position * mod + span) / (2 * span))
